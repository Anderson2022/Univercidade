...
export default class Studentscore implements JobContract {
  static all() {
    throw new Error('Method not implemented.')
  }
  public key = 'Studentscore'
  ...
  public async handle(job) {
    ...
    const { id } = data as IstudentScore

    const student = await Student.findByOrFail('id', id)


    const beneficts = await Benefit.findBy('student_id', student.id)
    const resultBenificts = beneficts != null
    console.log(resultBenificts);
    ...
    if (student.gender === "Feminino") {
      ...
      if (student.familyHouseHoldSize === "Entre 5 a 8 pessoas") {
        requirementIds.push(10);
